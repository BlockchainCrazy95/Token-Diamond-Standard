...
import { DiamondCutFacet, DiamondLoupeFacet, MintBurnFacet, OwnershipFacet, Test1Facet, MockCoinFacet } from "../build/typechain"
...
describe('DiamondTest', async function () {
  ...
  let MockCoinFacet:MockCoinFacet
  let mintBurnFacet:MintBurnFacet
  let tx
  let receipt
  let result
  ...
  before(async function () {
    ...
    MockCoinFacet = await ethers.getContractAt('MockCoinFacet', diamondAddress)
    mintBurnFacet = await ethers.getContractAt('MintBurnFacet', diamondAddress)
  })
  ...
  it('facets should have the right function selectors -- call to facetFunctionSelectors function', async () => {
    ...
    assert.sameMembers(result, selectors)
    selectors = getSelectors(MockCoinFacet)
    result = await diamondLoupeFacet.facetFunctionSelectors(addresses[3])
    assert.sameMembers(result, selectors)
    selectors = getSelectors(mintBurnFacet)
  ...
  it('selectors should be associated to facets correctly -- multiple calls to facetAddress function', async () => {
    ...
    assert.equal(
      addresses[3],
      await diamondLoupeFacet.facetAddress('0x70a08231')
    )
    assert.equal(
      addresses[3],
      await diamondLoupeFacet.facetAddress('0x23b872dd')
    )
    assert.equal(
      addresses[4],
      await diamondLoupeFacet.facetAddress('0x9dc29fac')
    )
  })
  ...
  it('should Mockcoin function call', async () => {
    await mintBurnFacet.mint(accounts[0].address, ethers.utils.parseEther("2000"));
    const balance = await MockCoinFacet.balanceOf(accounts[0].address)
    assert.equal(balance.toString(), ethers.utils.parseEther("2000").toString());
  })

  it("should only owner can call", async () => {
    await expect(mintBurnFacet.connect(accounts[0]).mint(accounts[0].address, ethers.utils.parseEther("1000"))).to.be.revertedWith("LibDiamond: Must be contract owner");
    await expect(mintBurnFacet.connect(accounts[0]).burn(accounts[0].address, ethers.utils.parseEther("1000"))).to.be.revertedWith("LibDiamond: Must be contract owner");
  })
  ...
  it('add most functions and facets', async () => {
    ...
    const cut = [
      ...
      },
      {
        ...
        action: FacetCutAction.Add,
        functionSelectors: getSelectors(MockCoinFacet)
      },
      {
        facetAddress: addresses[4],
        action: FacetCutAction.Add,
        functionSelectors: getSelectors(mintBurnFacet)
      },
      {
    ...
    assert.sameMembers(facets[findAddressPositionInFacets(addresses[3], facets)][1], getSelectors(MockCoinFacet))
    assert.sameMembers(facets[findAddressPositionInFacets(addresses[4], facets)][1], getSelectors(mintBurnFacet))
